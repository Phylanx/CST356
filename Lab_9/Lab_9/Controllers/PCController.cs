...
namespace Lab_9.Controllers
{
    public class PCController : Controller
    {
        ...
        {
            ViewBag.TotalStorage = TotalStorage(userId);
        ...
        private ICollection<PCViewModel> GetUserPCs(int userId)
        {
            ...
            foreach (var pc in _context.PCs)
            {
                if (pc.UserId == userId)
                {
        ...
        private ICollection<PCViewModel> GetAllPCs()
        {
            var userPCModels = new List<PCViewModel>();
            foreach (var pc in _context.PCs)
            {
        ...
        private int TotalStorage(int userId)
        {
            var gb = 0;
            foreach (var pc in GetUserPCs(userId))
            {
                gb += pc.GB_Storage;
            }
            return gb;
